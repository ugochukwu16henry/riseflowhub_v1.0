...
export function milestoneCompletedEmail(data: MilestoneCompletedData): { subject: string; html: string } {
  const name = (data.name as string) || 'there';
  const milestoneTitle = (data.milestoneTitle as string) || 'Milestone';
  const projectName = (data.projectName as string) || 'Your project';
  const content = `
    <h1 style="margin:0 0 16px;font-size:24px;">Milestone completed</h1>
