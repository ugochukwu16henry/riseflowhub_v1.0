...
export default function HirerHiresPage() {
  ...
  return (
    <div className="p-6 max-w-4xl">
      ...
      {loading ? <p className="text-gray-500">Loading...</p> : items.length === 0 ? <p className="text-gray-500">No hires yet. <Link href="/talent-marketplace" className="text-primary hover:underline">Browse talents</Link></p> : (
        <ul className="space-y-3">
          {items.map((h) => (
            <li key={h.id} className="rounded-xl border border-gray-200 bg-white p-4">
              <div className="font-medium">{h.projectTitle}</div>
              <div className="text-sm text-gray-600">Talent: {h.talent?.name} · {h.status}</div>
              <div className="mt-2 flex gap-2">
                <span className="rounded bg-gray-100 px-2 py-0.5 text-xs">{h.status}</span>
                {h.agreement && <Link href="/dashboard/hirer/agreements" className="text-sm text-primary hover:underline">Agreement</Link>}
              </div>
            </li>
          ))}
        </ul>
