...
| **“Agreements to Sign” / “Agreements” not found** | Section may be below fold, or only rendered when user has agreements. Admin “Agreements” link is there but **blocked by Welcome modal**. | Dismiss **Welcome** (and Setup if shown) in admin/client flows before looking for Agreements. For “Agreements to Sign”, use a **scroll into view** or make the assertion **optional** (e.g. only if visible). |
